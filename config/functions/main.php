...
####################################################################
  ...
  function generatePackageNumber(){
    ...
    while( $found !== true ){
      ...
      if ( !isset($_SESSION["packes"][$generated_reference]) )
      {
        $found = true;
      }
  ...
  function createPackage() {
    GLOBAL $_SESSION;
    $package = $_SESSION["current_package"];

    if ( !isset($_SESSION["packages"][$package]) )
    {
      $_SESSION["packages"][$package] = array();
      $_SESSION["packages"][$package]['services'] = array();
      $_SESSION["packages"][$package]['cost'] = 0;
      $_SESSION["packages"][$package]['discount'] = 0;
      $_SESSION["packages"][$package]['total'] = 0;
    }
  }
  ...
  function insertService($service) {
    ...
    if ( !empty(trim($service)) ) {
      $package = $_SESSION["current_package"];
      $_SESSION["packages"][$package]['services'][] = $service;

      end($_SESSION["packages"][$package]['services']);
      $key = key($_SESSION["packages"][$package]['services']);
